...
<div class="bbsView">
  ...
  <p class="viewInfo">
    <span class="num"><strong>NO.</strong> <?=$jb_idx?></span>
    <span class="date"><strong>DATE.</strong> <?=$jb_reg_date?></span>
    <span class="name"><strong>NAME.</strong> <?=$jb_name?></span>
    <span class="files"><strong>FILES.</strong>
		<?php								
		if($file_cnt > 0)
		{
			...
			for($i=0; $i<$file_cnt; $i++)	{
				if($ex_jb_file_name[$i]) {
					...
					if ($file_ext=="gif" || $file_ext=="jpg" || $file_ext=="png" || $file_ext=="bmp") {										
						$img_src .= "<a href='" . $GP->UP_IMG_SMARTEDITOR_URL ."jb_${jb_code}/${ex_jb_file_code[$i]}' target='_blank'>";
						$img_src .= "<img src=\"" . $GP->UP_IMG_SMARTEDITOR_URL ."jb_" . $jb_code . "/" . $ex_jb_file_code[$i] ."\" class='imgResponsive'>";						
						$img_src .= "</a><br />";
					}
				}	 
			}			
			$img_src= preg_replace_callback('#<img.+?src="([^"]*)".*?/?>#i', "url_encode_image", $img_src);
			echo $img_src;
		}
	?>
    <?=$content?>
