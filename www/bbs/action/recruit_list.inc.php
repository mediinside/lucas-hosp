...
if($data_list_cnt > 0) {
	for ($i=0; $i<$data_list_cnt; $i++ ) {		
			...
			$jb_title 					= $C_Func->strcut_utf8($data_list[$i]['jb_title'], 48, true, "...");	//제목 (길이, HTML TAG제한여부 처리)
			...
			if($jb_comment_count > 0) {
				$comment_count = "[${jb_comment_count}]";
			}else{
				$comment_count = "";
			}
			...
			$status = '';
			if($jb_etc4 == "N"){
				$status = "<span class='status'><strong>진행여부.</strong> 진행중</span>";
			}else {
				$status = "<span class='status end'><strong>진행여부.</strong> 마감</span>";
			}

			$updown_id = $jb_idx."_".$jb_group."_".$jb_depth;
			...
			echo ("
					<li class=\"" . $cls  . " rowdata\" id='" . $updown_id  ."'>
						<a href='" . $get_par . "'>
							<span class='num'><strong>등록일.</strong> " . $jb_reg_date. "</span>
							<span class='subject'><strong>제목</strong>". $jb_title ."</span>
							<span class='job'><strong>모집직종.</strong> ". $jb_etc1 ."</span>
							<span class='pos'><strong>직급.</strong> ". $jb_etc3 ."</span>
							" . $status. "
							<span class='date'><strong>마감일.</strong> ". $jb_etc2 ."</span>
						</a>
					</li>		
				");
