...
switch($_POST['mode']){
	...
	case 'DTG_AUTO_CH':
		...
		$rst = $C_Doctor->DTG_AUTO_CHAGE($args);
	...
	case "DOCTOR_GR_DEL" :
		...
		$args = "";
		$args['dtg_idx'] = $dtg_idx;
		$rst = $C_Doctor ->Doctor_GR_Info($args);
		if($rst) {
			@unlink($GP -> UP_DOCTOR . $rst['dtg_image']);
		}
		$rst1 = $C_Doctor ->Doctor_Gr_Del($args);
		echo "true";
		exit();
	break;

	case "DOCTOR_GR_IMGDEL":
			if (is_array($_POST)) foreach ($_POST as $k => $v) ${$k} = $v;
			
			$args = "";
			$args['dtg_idx'] = $dtg_idx;
			$rst = $C_Doctor -> Doctor_Gr_ImgDel($args);
	...
	case "DOCTOR_GR_MODI":
		...
		$args = "";
		$args['dtg_idx'] 		= $dtg_idx;
		$args['dtg_clinic'] 		= $dtg_clinic;
		$args['dtg_type'] 		= $dtg_type;
		...
		$args['dtg_image'] 	= $image_main;
		
		$rst = $C_Doctor -> Doctor_GR_Modi($args);
	...
	case "DOCTOR_GR_REG":
		...
		$rst = $C_Doctor -> Doctor_Gr_Reg($args);
	...
	case "DOCTOR_MODI" :
		...
		/*
		$dr_cl = "";
		if (is_array($dr_clinic)) {
			foreach ($dr_clinic as $k => $v) {
				$dr_cl .= $v . ",";
			}
		}*/
