<?php
	...
	$cn_select1 = $C_Func -> makeSelect_Normal('rc_hp_type', $GP -> HOSPITAL_TYPE1, $rc_hp_type, '', '::선택::');
?>
<body>
<div class="Wrap"><!--// 전체를 감싸는 Wrap -->
		...
		<div class="btnWrap">
			...
			<button onClick="layerPop('ifm_reg','./recruit_reg.php', '100%', 600)"; class="btnSearch btnRight">입사지원서 등록</button>
		</div>
							...
							<?
								...
								if($data_list_cnt > 0) {
									for ($i = 0 ; $i < $data_list_cnt ; $i++) {
										...
										$send_btn = "";
										if($rc_sms != 'Y') {
											$send_btn = $C_Button -> getButtonDesign('type2','접수문자',0,"send_sms('" . $rc_idx ."', '" . $rc_name. "')", 50, "title='접수문자 발송' ");	 
										}
										
										if($rc_sms == 'Y') {
											$send_btn = $C_Button -> getButtonDesign('type2','발송완료',0,"", 50,"title='접수문자 발송완료' ");	 
										}
										...
										if($rc_doc != '') {
											$code_file = $GP -> HOME. "upfile/recruit/" . $rc_doc;							
								...
								?>
									...
									echo "<tr><td colspan='10' align='center'>등록된 내역이 없습니다.</td></tr>";
								}
							?>						
						</tbody>
					</table>
