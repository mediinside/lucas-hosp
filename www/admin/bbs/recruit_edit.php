...
<div class="Wrap_layer"><!--// 전체를 감싸는 Wrap -->
	<div class="boxContent_layer">
		...
		<input type="hidden" name="before_image_main" id="before_image_main" value="<?=$rc_pic?>" />
		<input type="hidden" name="before_image_main_sub" id="before_image_main_sub" value="<?=$rc_doc?>" />
							...
							<td>
								...
								<?
									...
									if($rc_pic !=  '') {
										...
										echo $file_dn;
								?>
								<a href="#" onClick="img_del('<?=$rc_pic;?>','<?=$_GET['rc_idx']?>','P')">삭제(X)</a>
								<?
									}
								?>							
							</td>
							...
							<td>
								...
								<?
									if($rc_doc != '') {
										...
										$file_dn =  "<p><a href=\"/bbs/download.php?downview=1&file=" . $code_file . "&name=" . $rc_name . "_".$rc_doc ." \" class=\"btnL btnLs\">지원서 다운로드</a></p>";
										echo $file_dn;
								?>
								<a href="#" onClick="img_del('<?=$rc_doc;?>','<?=$_GET['rc_idx']?>','D')">삭제(X)</a>
								<?
									}
								?>
							</td>
...
<script type="text/javascript">
	...
	function img_del(image, idx, type)
	{
		...
		$.ajax({
			...
			data: "mode=RECRUIT_IMGDEL&rc_idx=" + idx + "&file=" + image + "&type=" + type,
			dataType: "text",
