...
switch($_POST['mode']){
	...
	case "RECRUIT_SMS" :
		...
		if($rst) {
			...
			$msg = $rc_name ."님 부민병원 입사지원이 접수되었습니다";
	...
	case "RECRUIT_IMGDEL":
		...
		$args['rc_idx'] = $rc_idx;
		$args['type'] = $type;
		$rst = $C_Recruit -> Recruit_ImgUpdate($args);

		@unlink($GP -> UP_RECRUIT . $file);
	...
	case "RECRUIT_REG":
			...
			if ($is_fileName) {
				...
				$image_main_sub = $updata['new_file_name'];	//변경된 파일명
			}else{
				$image_main_sub = $before_image_main_sub;
			}
			
			if($insertFileCheck) {
				$C_Func->put_msg_and_back($updata['error']);
			}
      
			//메인페이지 이미지 업로드
			$file_orName			= "rc_picture";
			$is_fileName			= $_FILES[$file_orName]['name'];
			$insertFileCheck	= false;
			if ($is_fileName) {
				$args_f = "";
			...
			$args = '';      
			$args['rc_pic']		= $image_main;
			$args['rc_title']		= $rc_title;
			$args['rc_doc']		= $image_main_sub;
			...
			$rst = $C_Recruit -> Recruit_Reg($args); 
