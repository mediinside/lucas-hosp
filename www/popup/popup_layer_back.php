	...
	for($i=0; $i<count($rst); $i++) {
		...
		if ($pop_type=="T") {
			...
			if($pop_link_url != "") {				
				...
				//$pop_str="<a href=\"javascript:move_page('${pop_link_url}')\"><img src='${url}' border='0' class='pop_img'></a>";$pop_str="<a href='" . $pop_link_url . "'><img src='${url}' alt='팝업' /></a>";
			} else {
	...
	<div id="divpop_<?=$pop_idx?>" style="position:absolute;top:5%;left:5%;right:5%;bottom:5%;z-index:9999;">
		...
		<form name="Pop_form_<?=$pop_idx?>">
			...
			<div style="height:30px;background:#000;font-size:12px; color:#FFFFFF;">
				<label style="float:left;padding:0 5px;"><input name="chkbox" type="checkbox" value="1" checked /> 이 창을 <?=$cookie_text?> 열지 않습니다.</label>
