...
CLASS ListClass
{
	...
	function getPageLink()
	{
		...
		if($total > 0)
		{			
			$rst ='<a href="?page='  . $prev . '&' . $tail . '" title="이전으로 이동하기"><span class="btn btn-default prev"><<</span></a> ';
			for($i = $start ; $i <= $end ; $i++){
				$pageLastclass = "";
				if ($i>=10) {
					$pageLastclass = "class=\"page_Last\"";
				}
				$rst .= ($i != $page)? "<a href='?page=" . $i . '&' . $tail . "'>":"<strong>";
				$rst .= ($i == $page)? "$i" : "$i";
				$rst .= ($i != $page)? "</a>":"</strong>";
				$rst .= ($i != $end) ? " " : "";
			}
			$rst .= ' <a href="?page=' . $this -> page_info['next'] . '&'. $tail . '" title="다음 페이지 이동하기"><span class="btn btn-default next">>></span></a>';
